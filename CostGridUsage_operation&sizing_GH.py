...
C_b_max = 50  # Max battery capacity in kWh 
...
model.addConstrs(E_b[i + 1] == E_b[i]* eff_b_sd + P_ch[i] * eff_b_ch - P_disch[i] / eff_b_disch for i in range(hours_in_year)) # Energy balance for the battery storage
model.addConstrs(P_disch[i] <= E_b[i] for i in range(hours_in_year)) # constraint for discharged power lower than available power
...
model.addConstrs(P_disch[i] / eff_b_disch + P_imp[i] >= E_dem[i] + P_ch[i]  for i in range(hours_in_year)) # Energy balance 
model.addConstr(E_b[hours_in_year] == E_b[0]) # periodicity
